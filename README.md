...
## Abstract
Meshtastic currently relies on simple flooding heuristics such as hop limits, duplicate suppression, randomized retransmission delays, and fixed signal thresholds. These approaches are robust and decentralized, but largely context-blind. A node deep inside a dense urban cluster and a node acting as a sparse bridge at the edge of coverage may apply identical retransmission behavior despite occupying fundamentally different positions within the propagation dynamics of the mesh. This leads to suboptimal flooding decisions, wasting bandwidth that could have been used to allow more hops.
This paper proposes framing retransmission as a learned probabilistic propagation problem. A lightweight neural policy is trained offline against propagation simulations that evaluate network-wide coverage outcomes under varying relay behaviors. Nodes receive a compressed representation of those probabilities that enables them to locally infer retransmission value from observable telemetry such as neighbor density, signal quality, congestion conditions, and recent propagation behavior.
The resulting system preserves the robustness and graceful degradation properties of conventional flooding while using airtime more selectively and efficiently. The expensive optimization occurs entirely offline. Deployment requires only compact local inference suitable for constrained hardware such as ESP32 devices. No topology dissemination, route maintenance, centralized coordination, or internet connectivity is required during operation.
## The Problem with Flooding
Anyone who has operated Meshtastic in a busy area has noticed the tradeoff inherent to flooding. In dense deployments, large fractions of airtime are consumed by redundant retransmissions. Multiple nearby nodes repeat packets that downstream receivers already obtained through other paths. In sparse deployments the opposite failure appears: suppression heuristics prevent retransmissions that were actually necessary for coverage continuity.
...
Traditional networking addresses this through routing. Nodes establish paths, maintain topology information, and forward packets toward specific destinations. But LoRa environments are poorly suited to classical routing. Links are intermittent, asymmetric, noisy, and highly load-dependent. Topology changes faster than low-bandwidth networks can disseminate updates. Airtime spent advertising routes is not available for payload traffic. Neighboring nodes sleep, move, disappear, and reappear unpredictably. The technology is meant for hikers that move.
...
A retransmission that reaches downstream nodes nobody else would have reached is very valuable. A retransmission that duplicates coverage already provided via another route is not. Packets still diffuse through the mesh opportunistically. The model only influences retransmission probability. The goal is improving propagation efficiency while preserving the robustness characteristics of flooding.
...
A node can observe: received signal strength and link quality, neighbor density, duplicate packet counts, retransmission timing patterns, local airtime congestion, hop counts, packet reception frequency, whether nearby neighbors already appear to be retransmitting, condensed fingerprints advertised by neighboring nodes' situation.
...
The system can be understood as operating on a probabilistic propagation field.
...
The model is not learning deterministic forwarding paths, but how local propagation conditions statistically relate to useful relay behavior.
## This Is Not Routing
...
Updated policy weights could be distributed through: firmware releases, optional mesh broadcasts, manual updates, or possibly even manual entry.
