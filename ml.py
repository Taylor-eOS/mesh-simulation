...
def features_to_vector(features):
    return [
        ...
        features["mean_out_signal"],
        features["mean_in_signal"],
...
def train(model, samples):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if epoch % EPOCH_INTERVAL == 0:
            print(
                f"ep={epoch}, mse={mse:.3f}, mae={mae:.3f}, r2={r2:.3f}, "
                f"{[round(w, 3) for w in model.weights]}, "
                f"bias={model.bias:.3f}"
            )
        if epochs_without_improvement >= patience:
            print(f"stabilized epoch={epoch}, best_mae={best_mae:.3f}, best_epoch={best_epoch}")
            break
...
def print_node_analysis(model, samples, fingerprints):
    for node, (_features, target) in enumerate(samples):
        ...
        print(
            ...
            f"targ={target:.3f}, "
            f"pred={pred:.3f}, "
            f"err={abs(pred - target):.3f}, "
            f"ut_var={fp['utility_variance']:.3f}, "
            f"cov={fp['mean_coverage']:.3f}, "
            f"red={fp['mean_redundancy']:.3f}"
        )
